...
def gammaCurve(classifier, gamValues, X, Y, Xtest, Ytest):
    M = len(gamValues)
    trainAcc = zeros(M)
    testAcc  = zeros(M)
    for m in range(M):
        # report what we're doing
        print "Training classifier with gambda=%g..." % (gamValues[m])
        
        # train the classifier
        classifier.setLambda(gamValues[m])
        classifier.reset()

        classifier.train(X, Y)
        y_hat = classifier.predict(Xtest)
        teAcc = 1 - computeAvgRegrMSError(Ytest, y_hat) 

        # store the results
        testAcc[m]  = teAcc

    return (gamValues, testAcc)

def gammaCurveSet(classifier, gamValues, X, Y, Xtest, Ytest):
    return gammaCurve(classifier, gamValues, X, Y, Xtest, Ytest)
...
def quantizeY(org_y, quantize_levels):
    ### TODO: YOUR CODE HERE
    org_y = array(org_y, dtype=float)
    return (org_y//quantize_levels)*quantize_levels
...
def computeAvgRegrMSError(org_y, predicted_y):
    ...
    '''
    ### TODO: YOUR CODE HERE
    org_y_m = array(org_y, dtype=float)

    diff_y = subtract(org_y_m, predicted_y)

    return mean(square(diff_y))
    #raise NotImplementedError
